...
- **Test expression / Test template** (in the alert editor) and the **expression
  ...
  a snapshot attached -- the "attach a webcam snapshot" checkbox, with either its own URL or
  the station-wide `default_image_url` from weewx.conf -- testing the template also fetches that frame and shows it, with
